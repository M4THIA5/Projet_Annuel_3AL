"use client"
import { getNewFriendByEmail, getProfile } from "#/lib/api_requests/user"
...
import { fetchFriends } from "./fetchFriend"
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    async function fetchProfileAndFriends() {
      const data = await getProfile()
      setProfile(data)
      if (data?.id) {
        const friendsData = await fetchFriends(data.id)
        setMultiFriendTypes(friendsData)
      }
    }
    ...
    <div className="flex min-h-screen bg-blue-50">
      ...
      {selectedTab === 'friends' && profile && (
          ...
          <CardContent>
            ...
            <ul className="space-y-2 max-h-64 overflow-y-auto pr-2">
              ...
              <li key={friend.id} className="flex items-center gap-2 p-2 rounded hover:bg-blue-100">
                <UserCard user={friend} profile={profile} setMultiFriendTypes={setMultiFriendTypes}/>
              </li>
              ))}
            </ul>
            ) : (
        ...
        <div className="flex w-full gap-6">
          ...
          <Card className="flex-1 shadow-lg border-0 bg-white/90">
          <CardHeader>
            <CardTitle className="text-xl font-semibold text-blue-200">Mes demandes d&apos;amis</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <div className="flex flex-col gap-4 mt-8">
          <Card className="shadow-lg border-0 bg-white/90">
            <CardHeader>
              <CardTitle className="text-xl font-semibold text-blue-200">Trouver de nouveaux amis</CardTitle>
                <div className="flex w-full max-w-sm items-center gap-2">
                  ...
                  type="email"
                  placeholder="Email"
                  aria-label="Rechercher un ami par email"
                  value={searchEmail}
            ...
            <CardContent>
            {newFriend ? (
              <UserCard user={newFriend} profile={profile} setMultiFriendTypes={setMultiFriendTypes}/>
            ): (
